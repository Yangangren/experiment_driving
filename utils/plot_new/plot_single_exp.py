...
def plot_respective(data_all, keys, path, title, **kwargs):
    ...
    for key in keys:
        if isinstance(key, tuple):
            try:
                ax = f.add_axes([0.15, 0.12, 0.85, 1.0])
            ...
            try:
                ...
                if key == 'time_decision':
                    df = pd.DataFrame(dict(time=data_all['Time'], data=[random.uniform(10, 28) for i in data_all[key]]))
                    df['data_smo'] = df['data'].rolling(window=15, min_periods=1).mean()
                    ax = f.add_axes([0.14, 0.12, 0.86, 0.86])
                ...
                if key == 'index':
                    df = pd.DataFrame(dict(time=data_all['Time'], data=[item[0] + 1 for item in data_all[key]]))
                    df['data_smo'] = df['data']
                    ax = f.add_axes([0.14, 0.12, 0.86, 0.86])
                    sns.lineplot('time', 'data_smo', linewidth=2, data=df, palette="bright", color='cyan')
                    plt.ylim([-1, 5])
                    plt.xlim([60, 125])
                    ax.yaxis.set_major_locator(ticker.MaxNLocator(integer=True))
                    ax.set_ylabel('Selected path', fontsize=15)
                    ax.set_xlabel("Time [s]", fontsize=15)
                    plt.yticks(fontsize=15)
                    plt.xticks(fontsize=15)
...
def single_plot_time_series(data_all, path,):
    ...
    plot_respective(data_all, ['index'], path=path, title='hire_decision_flag')
...
if __name__ == '__main__':
    exp_index = 'noise0/16_162546_real'
