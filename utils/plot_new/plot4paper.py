...
def single_plot(data_all, keys, path, title, smo, figname, lbs=None, **kwargs):
    ...
    index_list = search_automode_index(data_all)
    ...
    ax.add_patch(patches.Rectangle((time_s1, ylim[0]), time_e1-time_s1+1, ylim[1]-ylim[0], facecolor='green', alpha=1.0))
    ax.add_patch(patches.Rectangle((time_s2, ylim[0]), time_e2-time_s2+1, ylim[1]-ylim[0], facecolor='cyan', alpha=1.0))
...
def single_plot_time_series(data_all, path,):
    ...
    single_plot(data_all, ['SteerAngleAim'],
                path=path, smo=True, title='Steer Angle [$\circ$]',
                figname='steer_decision', lbs=None)
    single_plot(data_all, ['a_x'],
                path=path, smo=True, title='Acceleration [$\mathrm {m/s^2}$]',
                figname='acc_decision', lbs=None)
...
if __name__ == '__main__':
    exp_index = '/case2/noise0/11_140111_real'
