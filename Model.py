...
class PINet(nn.Module):
    def __init__(self, args):
        ...
        num_element = args.element_dim
        num_atom=args.atom_dim
        num_hidden_cell = args.num_hidden_cell
        self.NN_type = args.NN_type
        if self.NN_type == "CNN":
            self.conv_part = nn.Sequential(
                nn.Conv2d(num_element[-1], 32, kernel_size=4, stride=2, padding=3),
                nn.GELU(),
        ...
        self.output_layer = nn.Linear(num_hidden_cell, num_atom, bias=True)

        init_weights(self)

    def _get_conv_out_size(self, num_element):
        out = self.conv_part(torch.zeros(num_element).unsqueeze(0).permute(0, 3, 1, 2))
        return int(np.prod(out.size()))

    def forward(self, element):
        if self.NN_type == "CNN":
            x = self.conv_part(element)
            x = x.view(element.size(0), -1)
            x = self.linear1(x)
            x = F.gelu(x)
            x = self.linear2(x)
            x = F.gelu(x)
        elif self.NN_type == "mlp":
            x = self.linear1(element)
            x = F.gelu(x)
            x = self.linear2(x)
            x = F.gelu(x)
            x = self.linear3(x)
            ...
            x = self.linear5(x)
            x = F.gelu(x)
        atom = self.output_layer(x)
        return atom

    def evaluate(self, element):
        atom = self.forward(element)
        return atom
...
class QNet(nn.Module):
    def __init__(self, args,log_std_min=-0.1, log_std_max=4):
        ...
        self.linear1 = nn.Linear(num_states+num_action, num_hidden_cell, bias=True)
        self.linear2 = nn.Linear(num_hidden_cell, num_hidden_cell, bias=True)
        self.linear3 = nn.Linear(num_hidden_cell, num_hidden_cell, bias=True)
...
class PolicyNet(nn.Module):
    def __init__(self, args,log_std_min=-5, log_std_max=1):
        ...
        self.NN_type = args.NN_type
        self.args= args
        self.linear1 = nn.Linear(num_states, num_hidden_cell, bias=True)
        self.linear2 = nn.Linear(num_hidden_cell, num_hidden_cell, bias=True)
        self.linear3 = nn.Linear(num_hidden_cell, num_hidden_cell, bias=True)
        self.linear_mean_4 = nn.Linear(num_hidden_cell, num_hidden_cell, bias=True)
        self.linear_mean_5 = nn.Linear(num_hidden_cell, num_hidden_cell, bias=True)
